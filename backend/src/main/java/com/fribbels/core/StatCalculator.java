...
public class StatCalculator {
    ...
    private int speedSetBonus;
    private int revengeSetBonus;
    ...
    public void setBaseValues(final HeroStats base, final Hero hero) {
        ...
        speedSetBonus = (int) (0.25f * base.spd);
        revengeSetBonus = (int) (0.12f * base.spd);
    ...
    public float calcAtk(float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return ((bonusBaseAtk  + accs0[0]+accs1[0]+accs2[0]+accs3[0]+accs4[0]+accs5[0] + (sets[2] > 3 ? atkSetBonus : 0)) * bonusMaxAtk);
    }

    public float calcHp(float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return ((bonusBaseHp   + accs0[1]+accs1[1]+accs2[1]+accs3[1]+accs4[1]+accs5[1] + (sets[0] > 1 ? sets[0] / 2 * hpSetBonus : 0)) * bonusMaxHp);
    }

    public float calcDef(float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return ((bonusBaseDef  + accs0[2]+accs1[2]+accs2[2]+accs3[2]+accs4[2]+accs5[2] + (sets[1] > 1 ? sets[1] / 2 * defSetBonus : 0)) * bonusMaxDef);
    }

    public float calcCr(Hero hero, HeroStats base, float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return (base.cr  + accs0[6]+accs1[6]+accs2[6]+accs3[6]+accs4[6]+accs5[6] + (sets[4] > 1 ? sets[4] / 2 * 12 : 0) + hero.bonusCr + hero.aeiCr);
    }

    public int calcCd(Hero hero, HeroStats base, float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return (int) (base.cd    + accs0[7]+accs1[7]+accs2[7]+accs3[7]+accs4[7]+accs5[7] + (sets[6] > 3 ? 40 : 0) + hero.bonusCd + hero.aeiCd);
    }

    public int calcEff(Hero hero, HeroStats base, float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return (int) (base.eff   + accs0[8]+accs1[8]+accs2[8]+accs3[8]+accs4[8]+accs5[8] + (sets[5] > 1 ? sets[5] / 2 * 20 : 0) + hero.bonusEff + hero.aeiEff);
    }

    public int calcRes(Hero hero, HeroStats base, float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return (int) (base.res   + accs0[9]+accs1[9]+accs2[9]+accs3[9]+accs4[9]+accs5[9] + (sets[9] > 1 ? sets[9] / 2 * 20 : 0) + hero.bonusRes + hero.aeiRes);
    }

    public int calcSpeed(Hero hero, HeroStats base, float[] accs0, float[] accs1, float[] accs2, float[] accs3, float[] accs4, float[] accs5, int[] sets) {
        return base.spd   + (int)accs0[10]+(int)accs1[10]+(int)accs2[10]+(int)accs3[10]+(int)accs4[10]+(int)accs5[10] + (sets[3] > 3 ? speedSetBonus : (sets[14] > 3 ? revengeSetBonus : 0)) + hero.bonusSpeed + hero.aeiSpeed;
    }
    ...
    public HeroStats addAccumulatorArrsToHero(final HeroStats base,
        ...
        final float atk =  calcAtk(accs0, accs1, accs2, accs3, accs4, accs5, sets);
        final float hp =   calcHp(accs0, accs1, accs2, accs3, accs4, accs5, sets);
        final float def = calcDef(accs0, accs1, accs2, accs3, accs4, accs5, sets);
        final float cr =    calcCr(hero, base, accs0, accs1, accs2, accs3, accs4, accs5, sets);
        final int cd = calcCd(hero, base, accs0, accs1, accs2, accs3, accs4, accs5, sets);
        final int eff =  calcEff(hero, base, accs0, accs1, accs2, accs3, accs4, accs5, sets);
        final int res =  calcRes(hero, base, accs0, accs1, accs2, accs3, accs4, accs5, sets);
        final int spd =  calcSpeed(hero, base, accs0, accs1, accs2, accs3, accs4, accs5, sets);
    ...
    public HeroStats addAccumulatorArrsToHero2(final HeroStats base,
                                              final float[][] accs,
                                              final int[] sets,
                                              final Hero hero,
                                              final int upgrades,
...
//        return getNewStatAccumulatorArr(base, item, accumulatorsByItemId, useReforgeStats);
    }
